...
int mandelbrot_img(t_img img, int mouse_x, int mouse_y)
{
	int y;
	int x;
	double real;
	...
	while (y < HEIGHT + mouse_y)
	{
		...
		while (x < WIDTH + mouse_x)
		{
			...
			put_pixel_img(img, x - mouse_x, y - mouse_y,
						  map_color(mandelbrot_pix(real, imag, MAX_ITER) * 2, MAX_ITER));
			x++;
		}
		y++;
	}
...
int mandelbrot(void)
{
	...
	tutorial = new_program(WIDTH, HEIGHT, "Mandelbrot");
	if (!tutorial.win_ptr)
		return (2);
	img = new_img(WIDTH, HEIGHT, tutorial);
	vars()->win = &tutorial;
	vars()->img_ptr = &img;
	mandelbrot_img(img, 0, 0);
